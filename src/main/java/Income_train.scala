...
StructField("name", StringType, true),StructField("name_p", StringType, true),StructField("name_id", StringType, true),StructField("name_date", StringType, true),StructField("name_fromnumber", StringType, true)))
  ...
  val input_data = sqlContext.read.format("com.databricks.spark.csv").option("header", "false").option("delimiter","|").schema(customSchema).load("hdfs://master:9000/incomefinaldata")
...
sc.parallelize(Seq(model), 1).saveAsObjectFile("hdfs://master:9000/model/income_mm2")
