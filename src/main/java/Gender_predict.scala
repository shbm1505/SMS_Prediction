...
val a = sc.objectFile[PipelineModel]("hdfs://master:9000/model/gender_mm3").first()
val predictions=a.transform(data3)

val toGender = udf((gender: String) => {
  if (gender == "0.0") "male"
  else "female"
...
val y=predictions.withColumn("predict",toGender(predictions("predictedLabel")))
...
x.write
    ...
    .option("header", "true")
    .save("hdfs://master:9000/model/gendere_finaldatas7")
