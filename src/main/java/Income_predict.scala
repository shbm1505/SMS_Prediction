...
StructField("name", StringType, true),StructField("name_p", StringType, true),StructField("name_id", StringType, true),StructField("name_date", StringType, true),StructField("name_fromnumber", StringType, true)))
 ...
 //val customSchema = StructType(Array(StructField("gender", StringType, true),StructField("creditcount", StringType, true),StructField("withdrawlsum", StringType, true),StructField("e_expense", StringType, true),StructField("avg_brand_count", StringType, true),StructField("premium_brand_count", StringType, true),StructField("premiumsum", StringType, true),StructField("loansum", StringType, true),StructField("income", StringType, true),StructField("location", StringType, true),StructField("age", StringType, true)))
...
val new_df5=new_df.select("age2","phoneno","creditcount","withdrawlsum","e_expense","avg_brand_count","premium_brand_count","premiumsum","loansum","income2","gender2","location2")
*/val toDouble = udf[Double, String]( _.toDouble)

val new_df6=new_df5.filter("income2='NA'")
...
.withColumn("avg_brand_count", toDouble(new_df5("avg_brand_count")))
.withColumn("premium_brand_count",  toDouble(new_df5("premium_brand_count")))
.withColumn("premiumsum",      toDouble(new_df5("premiumsum"))) 
.withColumn("loansum", toDouble(new_df5("loansum")))
.withColumn("income2",  toDouble(new_df5("income2")))
.withColumn("age2",      toDouble(new_df5("age2"))) 
.withColumn("location2",      toDouble(new_df5("location2")))*/
...
val data2=data.withColumn("label",toDouble(data("income2")))


val data3=data2.select("features","phoneno")

val a = sc.objectFile[PipelineModel]("hdfs://master:9000/model/income_mm2").first()
val predictions=a.transform(data3)

val toIncome = udf((age: String) => {
  if (age == "1.0") "0-3L"
  else if(age=="2.0") "3-6L"
  else if(age=="3.0") "6-10L"
  else if(age=="4.0") "10-15L"
  else if(age=="5.0") "15-20L"
  else if(age=="6.0") "20-25L"
  else if(age=="7.0") "25-40L"
    else if(age=="8.0") "40-60L"
      else if(age=="9.0") "60L-1CR"
  else "1CR+"
...
x.write
    ...
    .option("header", "true")
    .save("hdfs://master:9000/model/incomee_finaldata")
